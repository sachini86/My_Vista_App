    ...
    if (_formKey.currentState!.validate()) {
      if (_artwork == null) {
        ScaffoldMessenger.of(
          context,
        ).showSnackBar(const SnackBar(content: Text('Please upload artwork')));
        return;
      }

      if (_formKey.currentState!.validate() && _artwork != null) {
        // Upload artwork
        String artworkUrl = await _uploadFile(_artwork!, 'artworks');
        ...
        Navigator.push(
          ...
          MaterialPageRoute(
            builder: (context) => ProductDetailPage2(
              artwork: _artwork!, // <-- required
              additionalFiles: _additionalFiles, // list of images/videos
              title: _titleController.text,
              artistName: _artistController.text,
              description: _descriptionController.text,
              category: _selectedCategory ?? '',
          ...
          Positioned(
            ...
            child: CircleAvatar(
              ...
              child: IconButton(
                ...
                onPressed: () {
                  setState(() {
                    _artwork = null; // remove only main artwork
                  });
                },
              ),
            ),
